...
class SaltInput:
    ...
    def input(self, input_name, input_desc, input_value, input_type, input_image=None, input_mask=None, unique_id=0):
...
class SaltOutput:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "output_name": ("STRING", {}),
                "output_desc": ("STRING", {}),
                "output_type": (["PNG", "GIF", "WEBP", "AVI", "MP4", "WEBM", "STRING"],),
    ...
    def output(self, output_name, output_desc, output_type, output_data, animation_fps=8, animation_quality="DEFAULT", unique_id=0):
        ...
        if output_type.strip() == "" or output_type not in ["GIF", "WEBP", "AVI", "MP4", "WEBM"]:
            if isinstance(output_data, torch.Tensor):
                output_type = "PNG"
            elif isinstance(output_data, str):
                output_type = "STRING"
            else:
                raise ValueError("Unsupported `output_type` supplied. Please provide `IMAGE` or `STRING` input.")
        ...
        output_path = os.path.join(OUTPUT, asset_id)
        ...
        if output_type in ["GIF", "WEBP", "AVI", "MP4", "WEBM"]:
            ...
            animator = ImageAnimator(output_data, fps=int(animation_fps), quality=animation_quality)
